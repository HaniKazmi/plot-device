...
describe("what a shelf is", () => {
  it("groups by a field all three media record, so none of them can vanish off the wall", () => {
    ...
    const items = toOmniItems(library({ game: [videoGame()], show: [showWithSeason()], movie: [movie()] }));
...
describe("shelving", () => {
  const items = () =>
    toOmniItems(
      library({
        ...
        movie: [movie({ name: "Arrival", genre: "Sci-Fi", minutes: 120 })],
      }),
    );
  ...
  it("stands a show on a shelf once, however many seasons of it there are", () => {
    ...
    const [group] = galleryGroups(toOmniItems(library({ show: [parent] })), "genre", "Items", "size", TODAY);
  ...
  it("still counts every season's hours behind that one card", () => {
    ...
    const [group] = galleryGroups(toOmniItems(library({ show: [parent] })), "genre", "Hours", "size", TODAY);
  ...
  it("stands it on each decade it was met in, since a shelf is collapsed within itself", () => {
    ...
    const groups = galleryGroups(toOmniItems(library({ show: [parent] })), "decade", "Items", "size", TODAY);
  ...
  it("shows a rewatched film once, where two viewings are two rows of one work", () => {
    const rewatched = library({
      movie: [movie({ startDate: YearMonthDay.get(2017, 1, 14) }), movie({ startDate: YearMonthDay.get(2021, 9, 2) })],
    });
...
describe("ordering the shelves", () => {
  ...
  const mixed = () =>
    toOmniItems(
      library({
        game: [videoGame({ name: "Halo", genre: "Shooter", hours: 40, endDate: YearMonthDay.get(2015, 3, 2) })],
        show: [spanningShow()],
        movie: [movie({ name: "Arrival", genre: "Sci-Fi", minutes: 120, startDate: YearMonthDay.get(2026, 2, 1) })],
      }),
    );
  ...
  it("dates a work by its last entry, not by the entry that fronts it", () => {
    ...
    const [drama] = galleryGroups(toOmniItems(library({ show: [spanningShow()] })), "genre", "Hours", "recent", TODAY);
  ...
  it("dates an item that has not closed as met now, since that is when it is being met", () => {
    ...
    const open = toOmniItems(
      library({
        ...
        movie: [movie({ name: "Arrival", genre: "Sci-Fi", startDate: YearMonthDay.get(2026, 2, 1) })],
      }),
    );
  ...
  it("leaves a show standing on each decade it was met in, whichever sort is on", () => {
    ...
    for (const sort of GALLERY_SORTS) {
      const groups = galleryGroups(toOmniItems(library({ show: [spanningShow()] })), "decade", "Items", sort, TODAY);
...
describe("books on the wall", () => {
  ...
  it("stands a reread book on a shelf once, where two reads are two rows of one work", () => {
    const items = toOmniItems(
      library({
        book: [book({ startDate: YearMonthDay.get(2020, 1, 1), endDate: YearMonthDay.get(2020, 1, 9) }), book()],
      }),
    );
