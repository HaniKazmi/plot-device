...
describe("movieSections", () => {
  it("runs the rail in the order the page does: Now, Vitals, Top, Explore, Timeline, Charts, Library", () => {
    expect(movieSections(true, false).map((section) => section.id)).toEqual([
  ...
  it("drops the Now chip when nothing survives the filters, so it never points at a missing anchor", () => {
    expect(movieSections(false, false).map((section) => section.id)).not.toContain(MOVIE_SECTIONS.latest);
  });

  it("keys the latest-watch anchor movie-latest whatever its chip reads", () => {
    expect(movieSections(true, false).map((section) => section.id)).not.toContain("movie-now");
  });

  it("names every anchor under MOVIE_SECTIONS exactly once when everything is offered", () => {
    const ids = movieSections(true, false).map((section) => section.id);
