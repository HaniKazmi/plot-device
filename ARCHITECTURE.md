...
- **Eight React Compiler bailouts** remain (§7), all on the same limit: a destructured prop with a default value opts its component out. Three are in `common/Card.tsx`, which every media card in the app renders, so this is the one bailout that is on a hot path.
