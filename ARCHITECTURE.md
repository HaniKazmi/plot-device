...
belongs beside it. `CardMediaImage` takes the bar colour as `chromeColour`, which paints the
collapsed card alone and leaves `colour` — given, or sampled — to the expanded one.
...
and each tab's measure in the section rail — the last through `MeasureControl`, which owns the
wiring to the filter reducer once for the five tabs. Values that are already their own words —
a measure, a view, a density — become segments through `common/segments.ts` rather than a
written-out pair each. Words rather than icons, because an icon states a
...
the 22px chip rail it sits in would read as two controls rather than one relocating. The Shows
timeline's choice of one bar per season or per show is the same kind of choice and takes the same
control, where a switch labelled "Combine Seasons" states only the state it is not in. The
franchise strip's Order · Time switch is the caller that passes a `tone`: the theme's primary is
solved against the theme's paper, and on a card the artwork has coloured the lit segment can land a
hue from legible — a teal on a brown card. On a toned control the lit segment is filled with the
surface's ink and its word set in the ground, the strongest pair the surface has, and the unlit
words stand in the ink at full strength rather than the muted tone, which is a transparent ink and
on a mid-toned ground differs from it by too little for a 12px word to carry. `SelectBox`
...
The library grids are hundreds of banner cards deep with nothing between them, so a reader scrolled into one has no way to tell where in the sort order they are. A pill fixed just under the section rail names it: the year of the topmost visible row under the date sort, that row's franchise initial under the franchise sort, and the figure — a score, a hundred of pages — under a sort the domain adds. Those are `FinishedExtraSort`s, an order over a figure only the domain holds, offered after the two every wall has: Movies adds Score, Books adds Score and Pages. An order belongs to the wall rather than to a strip of its own, because "what was best" is the same library read in another order and a wall is where a whole order can be read, where a strip shows six. Highest first, and an item with no figure last rather than first — a film never scored is not the best of anything. The two built-in names are reserved, and the wall refuses an extra that takes one: the built-ins answer first in both `finishedItems` and `finishedBucket`, so a name shared between them would sort the wall one way and label the marker another. `finishedBucket` derives the label from the same field `finishedItems` orders by — through the same `franchiseKey`, so a work with no franchise recorded falls back to its own title in both places — so the marker and the wall cannot disagree about which field is in play, and it answers `null` for a value with no short form — an undated item, which the date sort places first, so the topmost card genuinely can be one.
...
Franchise strip) in its hero variant, in the order met. The strip is the one thing on the page that
places the item among its series, where the ledger facts it could hold instead — a release date, a
developer — wait a click away in the expanded card; the Shows kicker names the episode in hand,
since the title is the show's name and nothing else on the hero says which season it is on. The
strip fits because the panel is what the artwork's height leaves and the hero variant never asks
for more: a caption line, one row of beads and a row of years is under a third of it, and a series
too long for the pitch closes its beads up on that one row rather than turning onto a second,
which would stand the panel taller than the artwork and letterbox the picture inside its own card.
Between `md` and `lg` the strip is dropped, where a wrapped title beside a banner leaves it no room.
...
publishes. It carries no corner chip: a thumbnail wears one because it has no room for the word,
and everything a chip abbreviates — the platform, the latest episode, the score — is already in
the panel beside the picture, which for the platform carries the swatch its charts wear. Reading the accent any other way would mean sampling the same image twice and painting
