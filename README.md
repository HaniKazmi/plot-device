...
- **Search and filters as one box** — ⌘K or `/` finds a work, a franchise or an attribute across all four libraries; the section rail's own chip opens the same box on the current page's settings and filters. A genre typed in Find opens every game, show, film and book carrying it, narrows the page in place, or takes you to the tab that records it with the filter already set.
