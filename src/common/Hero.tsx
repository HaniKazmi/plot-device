...
/**
 * What the panel's middle holds beside the artwork. `rows` is the two ledger facts; `strip` is the
 * franchise strip in the panel, with the ledger left to the expanded card; `both` seats the rows
 * in the panel and the strip as a band beneath the row.
 */
type HeroMiddle = "rows" | "strip" | "both";
const HERO_MIDDLE: HeroMiddle = "strip";
...
export const Hero = <T,>(props: {
  ...
  // Widened back to the union: the constant is narrowed to its literal at every reference, which
  // would make each branch below for the other two arrangements a type error rather than a choice.
  const middle = HERO_MIDDLE as HeroMiddle;
  const rows = middle !== "strip" && props.rows && props.rows.length > 0 && (
  ...
  // Dropped between `md` and `lg`, where the column beside a banner wraps the title and a wrapped
  // title with the strip outgrows the picture beside it, letterboxing it inside its own card.
  const stripInPanel = middle === "strip" && props.strip && (
    <Box sx={{ display: { xs: "block", md: "none", lg: "block" } }}>{props.strip}</Box>
  );
  const stripBeneath = middle === "both" && props.strip && (
      ...
      footerComponent={
        <>
          <CardPanel
            ...
            middle={rows || stripInPanel || undefined}
          />
          {stripBeneath}
        </>
      }
