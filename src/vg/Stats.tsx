...
import { Card, CardHeader, CardContent, Typography, List, ListItem, ListItemText, Divider, Stack, CardMedia } from "@mui/material";
...
const AverageTimePerGame = ({ data }: { data: VideoGame[] }) => {
    const total = format(Math.round(data.filter((game) => game.hours).reduce((pre, cur) => pre + parseInt(cur.hours!), 0) / data.length))
    return <StatCard icon={<ShowChart />} title="Avg Time per Game" content={`${total} Hours`} />
}

const ThisYearSoFar = ({ data }: { data: VideoGame[] }) => {
    const total = format(data.filter((game) => game.startDate?.getFullYear() === new Date().getFullYear() && game.hours).reduce((pre, cur) => pre + parseInt(cur.hours!), 0))
    return <StatCard icon={<Update />} title="This Year So Far" content={`${total} Hours`} />
}

const RecentlyComplete = ({ data }: { data: VideoGame[] }) => {
    const recent = data.filter(a => a.hours && a.startDate && a.endDate).sort((a, b) => a.endDate! < b.endDate! ? 1 : -1).slice(0, 6);
    return <StatList icon={<Pause />} title="Recently Finished" content={recent} />
}

const MostPlayed = ({ data }: { data: VideoGame[] }) => {
    const most = data.filter(a => a.hours && a.startDate && a.endDate).sort((a, b) => parseInt(a.hours!) < parseInt(b.hours!) ? 1 : -1).slice(0, 6);
    ...
    return (
        <Grid xs={12} md={4}>
            ...
            <Card sx={{flex: "0 1 auto"}}>
                <CardHeader titleTypographyProps={{ variant: 'h6' }} title="Currently Playing" subheader={`Started ${recent.startDate?.toLocaleDateString()}`} avatar={<PlayArrow />} />
                <CardMedia component="img" src="https://images.launchbox-app.com/593bbec1-bacd-4d34-b67e-debbdc892286.jpg" /> 
            </Card>
            <Card sx={{flex: "1 1 auto"}}>
                <CardHeader titleTypographyProps={{ variant: 'h6' }} title="Next Up" subheader={`Releases ${new Date("11/10/2022").toLocaleDateString()}`} avatar={<SkipNext />} />
...
const StatList = ({ icon, title, content }: { icon: JSX.Element & React.ReactNode, title: string, content: VideoGame[] }) => {
    return (
        <Grid xs={6} md={4}>
            <Card sx={{ height: "100%" }} >
                    ...
                    <List>
                        {content.map(game => (
                            <Fragment key={game.game}>
                                ...
                                <ListItem>
                                    <ListItemText secondaryTypographyProps={{ component: 'span' }} primary={game.game} secondary={
...
const StatCard = ({ icon, title, content }: { icon: JSX.Element & React.ReactNode, title: string, content: string }) => {
    return (
        <Grid xs={6} md={3}>
            <Card sx={{ height: "100%" }} >
                <CardHeader titleTypographyProps={{ variant: 'h6' }} title={title} avatar={icon} />
