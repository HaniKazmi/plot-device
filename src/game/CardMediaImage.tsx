...
const GameCardDetail = ({ item }: { item: VideoGame }) => {
...
const gameEntries = (games: VideoGame[], today: YearMonthDay): FranchiseEntry[] =>
  games.map((game) => gameEntry(game, today, () => <GameHoverCard item={game} />));
 ...
 */
export const GameHoverCard = ({ item }: { item: VideoGame }) => {
  const scheme = useScheme();
...
export default GameCardMediaImage;
