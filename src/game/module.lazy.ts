 ...
 * this file is where they stay out of it. The election and the panel (`now.ts`, re-exported here)
 * ride in this chunk for the same reason and one more: what they answer is a card — the one the
 * composing tab's Now band leads with — stated in this tab's own words rather than in a fifth
 * vocabulary.
 *
 * These three and nothing else. The lookup over the four is dynamic (`MEDIA_LAZY[item.medium]`), so
 ...
 */
export { default as CardMediaImage, GameHoverCard as HoverCard } from "./CardMediaImage";
export { now } from "./now";
